package com.fcy.job.core.rpc.netcom.jetty.server;

import com.fcy.job.core.thread.ExecutorRegistryThread;
import com.fcy.job.core.thread.TriggerCallbackThread;
...
public class JettyServer {
		...
		thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					// Start server
					server.start();
					logger.info(">>>>>>>>>>> fcy-job jetty server start success at port:{}.", port);
					...
					logger.info(">>>>>>>>>>> fcy-rpc server join success, netcon={}, port={}", JettyServer.class.getName(), port);
				} catch (Exception e) {
	...
	public void destroy() {
		...
		logger.info(">>>>>>>>>>> fcy-rpc server destroy success, netcon={}", JettyServer.class.getName());
	}
